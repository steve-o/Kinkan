...
kinkan::upa_t::upa_t (const config_t& config) :
	config_ (config)
{
}
...
/* One may choose to use rsslInitializeEx to set additional options such as
 * OpenSSL dynamic libs.
 */
bool
kinkan::upa_t::Initialize()
{
	RsslError rssl_err;
 ...
 *
 * No locking for single thread operation.
 */
	VLOG(2) << "Initializing UPA.";
	if (RSSL_RET_SUCCESS != rsslInitialize (RSSL_LOCK_GLOBAL /*_AND_CHANNEL*/, &rssl_err)) {
