...
kinkan::provider_t::provider_t (
	const kinkan::config_t& config,
	std::shared_ptr<kinkan::upa_t> upa,
	kinkan::client_t::Delegate* request_delegate 
	) :
	...
	is_pending_directory_update_ (false),
...
/* 9.4.1. Bind server socket. */
	...
	addr.serviceName	     = const_cast<char*> (config_.downstream_rssl_port.c_str());	// port or service name
...
bool
kinkan::provider_t::WriteRawClose (
...
bool
kinkan::provider_t::DoInternalWork()
{
	bool did_work = false;
...
/* tbd: throttle directory updates to not overload infrastructure. */
		if (is_pending_directory_update_.exchange (false)) {
			DVLOG(3) << "Applying directory update to all active connected clients.";
			for (auto it = connections_.begin(); it != connections_.end(); ++it) {
				RsslChannel* c = *it;
				if (nullptr != c->userSpecPtr && RSSL_CH_STATE_ACTIVE == c->state) {
					auto client = reinterpret_cast<client_t*> (c->userSpecPtr);
...
void
kinkan::provider_t::ScheduleDelayedWork (
 ...
 */
void
kinkan::provider_t::OnConnection (
...
void
kinkan::provider_t::RejectConnection (
	RsslServer* rssl_sock
	)
{
#ifndef NDEBUG
...
void
kinkan::provider_t::AcceptConnection (
	RsslServer* rssl_sock
	)
{
#ifndef NDEBUG
...
void
kinkan::provider_t::OnCanReadWithoutBlocking (
	RsslChannel* c
	)
{
	DCHECK (nullptr != c);
...
void
kinkan::provider_t::OnInitializingState (
	RsslChannel* c
	)
{
...
void
kinkan::provider_t::OnCanWriteWithoutBlocking (
	RsslChannel* c
	)
{
	RsslError rssl_err;
 ...
 */
void
kinkan::provider_t::OnActiveClientSession (
	RsslChannel* c
	)
{
	DCHECK (nullptr != c);
...
void
kinkan::provider_t::OnActiveState (
	RsslChannel* c
	)
{
	RsslBuffer* buf;
...
void
kinkan::provider_t::RejectClientSession (
	RsslChannel* handle,
	const char* address
	)
{
...
bool
kinkan::provider_t::AcceptClientSession (
	RsslChannel* handle,
	const char* address
	)
{
 ...
 */
bool
kinkan::provider_t::GetDirectoryMap (
 ...
 */
bool
kinkan::provider_t::GetServiceDirectory (
	RsslEncodeIterator*const it,
	const char* service_name,	/* nullptr for all services */
	uint32_t filter_mask
	)
...
bool
kinkan::provider_t::GetServiceFilterList (
	RsslEncodeIterator*const it,
	uint32_t filter_mask
	)
{
 ...
 */
bool
kinkan::provider_t::GetServiceInformation (
	RsslEncodeIterator*const it
	)
{
#ifndef NDEBUG
 ...
 */
	...
	data_buffer.data   = const_cast<char*> (config_.downstream_service_name.c_str());
	data_buffer.length = static_cast<uint32_t> (config_.downstream_service_name.size());
 ...
 */
bool
kinkan::provider_t::GetServiceCapabilities (
	RsslEncodeIterator*const it
	)
{
#ifndef NDEBUG
...
bool
kinkan::provider_t::GetServiceDictionaries (
	RsslEncodeIterator*const it
	)
{
#ifndef NDEBUG
...
bool
kinkan::provider_t::GetServiceQoS (
	RsslEncodeIterator*const it
	)
{
#ifndef NDEBUG
 ...
 */
bool
kinkan::provider_t::GetServiceState (
	RsslEncodeIterator*const it
	)
{
#ifndef NDEBUG
 ...
 */
	...
	element.dataType   = RSSL_DT_UINT;
	const uint64_t is_accepting_requests = is_accepting_requests_.load() ? 1 : 0;
 ...
 */
bool
kinkan::provider_t::GetServiceLoad (
	RsslEncodeIterator*const it
	)
{
#ifndef NDEBUG
